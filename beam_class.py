...
class Beam:
    ...
    def from_beam_waist(cls, waist_radius, wavelength=1550e-09, power=1.0, M2=1.0):
        ...
        if wavelength is None:
            raise ValueError(
                "Wavelength must be provided to calculate the Rayleigh range."
            )
    ...
    def get_waist(self, z=None):
        ...
        return self.waist_radius * np.sqrt(
            1 + (self.M2 * (z / self.rayleigh_range)) ** 2
        )
    ...
    def get_cartesian_intensity(self, x, y, z):
        """Calculates the intensity I(x, y, z) at Cartesian coordinates (x, y, z)."""
        r = np.sqrt(x**2 + y**2)
    ...
    def propagate_to_waist(self):
        """Propagates the beam to its waist position."""
